...
def _smoothstep(edge0: float, edge1: float, x: float) -> float:
    """
    Smooth S-curve interpolation between 0 and 1, used to model gradual
    physiological onset/offset (insulin action, carb absorption) instead
    of abrupt linear jumps. Standard cubic smoothstep: 3x² - 2x³.
    """
    if edge0 == edge1:
...
# Approximate onset / peak / end-of-action windows (in minutes) for each
# insulin type, based on typical published pharmacokinetic profiles.
# These are illustrative ranges for an educational model — real onset/
# peak/duration varies by individual and is not something this app
# diagnoses or prescribes around.
...
# How quickly each diabetes type's own body brings a glucose excursion
# back toward baseline, independent of injected insulin (i.e. residual
# natural insulin response). Shorter decay = faster natural correction.
CARB_RESPONSE_PROFILE = {
    "No Diabetes":      {"peak": 45, "decay": 75},
    "Prediabetes":      {"peak": 50, "decay": 100},
...
def _cumulative_insulin_fraction(minutes: float, onset: float, peak: float, end: float) -> float:
    """
    Fraction (0–1) of an insulin dose's total glucose-lowering effect that
    has already been delivered by `minutes` after injection. Modeled as two
    smoothstep ramps: onset→peak covers the first half of the effect,
    peak→end covers the second half — giving a realistic S-shaped action
    curve instead of a flat percentage table.
    """
    if minutes <= onset:
        return 0.0
    if minutes <= peak:
...
def _carb_excursion_fraction(minutes: float, peak: float, decay: float) -> float:
    """
    Relative size (0–1) of the carb-driven glucose excursion at a given time:
    rises smoothly to its peak around `peak` minutes, then eases back down
    toward baseline over the following `decay` minutes as the body (and any
    injected insulin, handled separately) clears the glucose.
    """
    if minutes <= peak:
...
def glucose_prediction_model(
    ...
    """
    Educational glucose prediction model for 30/60/90/120-minute timepoints.

    Combines three physiologically-inspired terms:
      1. Carb excursion — glucose rise from the meal, shaped as a smooth
         rise-then-decay curve whose peak size and return-to-baseline speed
         depend on the diabetes type's natural insulin response.
      2. Injected insulin — glucose-lowering effect shaped by an onset/peak/
         end-of-action curve specific to the insulin type, correctly
         accounting for time already elapsed since injection (so insulin
         taken hours ago contributes little remaining future effect).
      3. Weight-adjusted sensitivity — lighter patients are modeled as
         somewhat more insulin-sensitive than heavier patients, a common
         simplifying heuristic (reference weight 70 kg), clamped to a
         reasonable range so extreme weights don't distort results.

    This is a simplified simulation for demonstration purposes, not a
    clinical dosing or prediction algorithm.
    """
    # ── 1. Carb excursion (glucose-raising effect of the meal) ──
    carb_factor = {"No Diabetes": 2.0, "Prediabetes": 3.0,
                   "Type 1 Diabetes": 5.0, "Type 2 Diabetes": 4.0}.get(diabetes_type, 3.0)
    carb_peak_rise = carbs_g * carb_factor
    carb_profile = CARB_RESPONSE_PROFILE.get(diabetes_type, CARB_RESPONSE_PROFILE["Prediabetes"])

    # ── 2. Injected insulin (glucose-lowering effect) ──
    insulin_factor_map = {
        "No Insulin": 0,
    ...
    base_factor_per_unit = insulin_factor_map.get(insulin_type, 0)

    # Weight-adjusted sensitivity: lighter patients see a slightly larger
    # drop per unit, heavier patients a slightly smaller drop per unit.
    weight_kg = weight_kg if weight_kg and weight_kg > 0 else 70.0
    weight_adjustment = max(0.6, min(1.6, 70.0 / weight_kg))
    factor_per_unit = base_factor_per_unit * weight_adjustment
    ...
    # ── 3. Project both effects forward to each future timepoint ──
    predictions = {}
    for t in (30, 60, 90, 120):
        # Carb excursion at this future point (time is measured from the meal)
        carb_delta = carb_peak_rise * _carb_excursion_fraction(
            t, carb_profile["peak"], carb_profile["decay"]
        )

        # Insulin's REMAINING effect between now and time t — i.e. how much
        # more of the dose's action will be delivered from this point
        # forward, correctly accounting for time already elapsed since
        ...
        remaining_fraction = max(0.0, delivered_by_t - already_delivered)
        insulin_delta = total_insulin_effect * remaining_fraction

        predicted = current_glucose + carb_delta - insulin_delta
        # Clamp to a physiologically plausible display range
        predictions[t] = round(max(40.0, min(600.0, predicted)), 1)
